...
public:
    ...
    //Import single SYM( -sxl)
    void ImportSymbol( wxFileName& fn, wxString& fileFullPath );
    //Import multi SYM( -sxl)
    void ImportSymbol( wxFileName& fn, wxString& fileFullPath, wxString filenameLC );

    //Ixport single SYM LibraryFile( -sxl,not used)
    wxString AddLibraryFile( bool aCreateNew, wxFileName& fn );
    
    //Export single SYM( -sxl)
    void ExportSymbol( LIB_SYMBOL* aSymbol, wxString& fileSaveFullPath );
    //Export multi SYM( -sxl)
    void ExportSymbolNoPopDialog( LIB_SYMBOL* aSymbol, wxString outDir, wxString filenameLC );

    //wx Open Selected File dialog( -sxl)
    wxFileName OpenFileDialog( const wxString& aLastPath );
