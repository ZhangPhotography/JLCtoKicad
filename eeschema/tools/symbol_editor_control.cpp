...
int SYMBOL_EDITOR_CONTROL::AddSymbol( const TOOL_EVENT& aEvent )
{
 ...
 //Analyse single LC SYM( -sxl)
int SYMBOL_EDITOR_CONTROL::AnalyseLCEDASYM( const TOOL_EVENT& aEvent )
{
    //刷新画布
    m_frame->RefreshCanvas();
    std::string strInFileFullPath = "";
    wxString lastOpenFile = "";
    wxFileName  fn = static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->OpenFileDialog( lastOpenFile );

    LCSYMtoKicadSYM SYMT;
    if( fn.GetFullPath().Contains( ".json" ) )
    {
        SYMT.importLCSYM( fn.GetFullPath().ToStdString(), 1 );
    }
    else
    {
        SYMT.importLCSYM( fn.GetFullPath().ToStdString() );
    }

    LIB_SYMBOL* fs = SYMT.m_symbolObj;
    if( fs )
    {
        wxString strFileSaveFullPath = "";
        static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->ExportSymbol( fs, strFileSaveFullPath );

        //static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->SetCurSymbol( fs, true );
        LIB_SYMBOL* curSymbol = static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->GetCurSymbol();
        LIB_FIELD*  field_ref = curSymbol->GetFieldById( 0 );
        field_ref->SetText( "." );
        field_ref->SetPosition( VECTOR2I( 0.0, 0.0 ) );
        updateItem( field_ref, true );
        LIB_FIELD* field_val = curSymbol->GetFieldById( 1 );
        field_val->SetText( "" );
        field_val->SetPosition( VECTOR2I( 0.0, 0.0 ) );
        updateItem( field_val, true );
        //saveCopyInUndoList( fs, UNDO_REDO::LIBEDIT );

        //static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->GetCanvas()->Refresh();
        //static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->OnModify();
        //static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->UpdateSymbolMsgPanelInfo();

        //static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->Save();
        //to save picture
        strFileSaveFullPath.Replace( ".kicad_sym", ".jpg" );
        static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->Zoom_Automatique( true );
        static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->RefreshCanvas();
        SaveCanvasImageToFile( static_cast<SYMBOL_EDIT_FRAME*>( m_frame ), strFileSaveFullPath,
                               BITMAP_TYPE::JPG );

        SYMT.changePROPERTY( curSymbol );
        updateItem( field_ref, true );
        updateItem( field_val, true );
        static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->Save();
        static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->Zoom_Automatique( true );
...
//Analyse multi LC SYM( -sxl)
int SYMBOL_EDITOR_CONTROL::MultiAnalyseLCEDASYM(const TOOL_EVENT& aEvent)
{
    wxString msg;
    wxString selectImportDir =
            static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->OpenDirDialog( "Select Import Direction" );
    wxArrayString files =
            static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->GetAllFilesInDir( selectImportDir );
    wxString selectExportDir =
            static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->OpenDirDialog( "Select Export Direction" );
    ...
    for( int i = 0; i < files.size(); i++ )
    {
        try
        {
            ...
            {
                SYMT.importLCSYM( files[i].ToStdString(), 1 );
            }
            else
            {
                SYMT.importLCSYM( files[i].ToStdString() );
            }
            ...
            if( fs )
            {
                static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->ExportSymbolNoPopDialog(
                        fs, selectExportDir, files[i] );
            }
            //to modify FIELDs
            static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->SetCurSymbol( fs, true );
            LIB_FIELD* field_ref = fs->GetFieldById( 0 );
            field_ref->SetText( "." );
            field_ref->SetPosition( VECTOR2I( 0.0, 0.0 ) );
            updateItem( field_ref, true );
            LIB_FIELD* field_val = fs->GetFieldById( 1 );
            field_val->SetText( "" );
            field_val->SetPosition( VECTOR2I( 0.0, 0.0 ) );
            updateItem( field_val, true );
            ...
            //to save picture
            static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->Zoom_Automatique( true );
            static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->UpdateWindowUI();
            static_cast<SYMBOL_EDIT_FRAME*>( m_frame )->RefreshCanvas();
            //std::this_thread::sleep_for( std::chrono::milliseconds( 100 ) );
            //SaveCanvasImageToFile( static_cast<SYMBOL_EDIT_FRAME*>( m_frame ),
            //                       selectExportDir + "/" + fs->GetName() + ".jpg",
            //                       BITMAP_TYPE::JPG );
            std::thread saveImage(SaveCanvasImageToFile,
                                   static_cast<SYMBOL_EDIT_FRAME*>( m_frame ),
                    selectExportDir + "/" + fs->GetName() + ".jpg",
                                   BITMAP_TYPE::JPG );
        ...
        {
            msg.Printf( _( "Failed to transform symbol library file '%s'." ), files[i] );
            DisplayErrorMessage( nullptr, msg, ioe.What() );
            return 0;
        }
    }

    msg = wxString::Format( _( "Symbol exported to file End" ) );
    DisplayInfoMessage( m_frame, msg );
    return 0;
}
...
void SYMBOL_EDITOR_CONTROL::setTransitions()
{
    ...
    Go( &SYMBOL_EDITOR_CONTROL::AnalyseLCEDASYM,       EE_ACTIONS::analyseLCEDASYM.MakeEvent() );
    Go( &SYMBOL_EDITOR_CONTROL::MultiAnalyseLCEDASYM,  EE_ACTIONS::multianalyseLCEDASYM.MakeEvent() );
