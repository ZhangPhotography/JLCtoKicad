...
wxFileName SCH_EDIT_FRAME::OpenFileDialog( const wxString& aLastPath, int type )
{
    static int lastFilterIndex = 0; // To store the last choice during a session.
    wxString   wildCard;

    wildCard << "Scalable Vector Graphics(*svg)|*.svg" << wxChar( '|' ) << "AutoCAD DXF(*.dxf)|*.dxf"
             << wxChar( '|' ) << " All supported formats( *.*)|*.*";

    wxFileDialog dlg( this, _( "Import SVG/DXF" ), aLastPath, wxEmptyString, wildCard,
