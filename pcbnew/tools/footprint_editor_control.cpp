...
bool FOOTPRINT_EDITOR_CONTROL::Init()
{
    ...
    ctxMenu.AddSeparator();
    ctxMenu.AddItem( PCB_ACTIONS::analyseLCEDAFP, SELECTION_CONDITIONS::ShowAlways );
    ctxMenu.AddItem( PCB_ACTIONS::multiAnalyseLCEDAFP, SELECTION_CONDITIONS::ShowAlways );
...
//analyse single FP( -sxl )
int FOOTPRINT_EDITOR_CONTROL::AnalyseLCEDAFP( const TOOL_EVENT& aEvent )
{
    //清理图形缓存
    m_frame->Clear_Pcb( true );
    m_frame->RefreshCanvas();

    wxString lastOpenFile = "";
    wxFileName fn = m_frame->OpenFileDialog( lastOpenFile );

    LCFPChangeToKicadFP FPT( m_frame->GetBoard() );
    if( fn.GetFullPath().Contains( ".json" ) )
    {
        FPT.importLCFP( fn.GetFullPath().ToStdString(), 1 );
    }
    else
    {
        FPT.importLCFP( fn.GetFullPath().ToStdString() );
    }

    FOOTPRINT* fp = FPT.m_KicadFPObj;
    if( fp )
    {
        wxString strFileSaveFullPath = "";
        m_frame->ExportFootprint( fp, strFileSaveFullPath );

        //加载FP
        m_frame->ImportFootprint( strFileSaveFullPath );
        strFileSaveFullPath.Replace( ".kicad_mod", ".jpg" );

        m_frame->Zoom_Automatique( false );
        m_frame->RefreshCanvas();
        SaveCanvasImageToFile( m_frame, strFileSaveFullPath, BITMAP_TYPE::JPG );
    }

    return 0;
}

//analyse multy FP( -sxl )
int FOOTPRINT_EDITOR_CONTROL::MultiAnalyseLCEDAFP( const TOOL_EVENT& aEvent )
{
    wxString      selectImportDir = m_frame->OpenDirDialog( "Select Import Direction" );
    wxArrayString files = m_frame->GetAllFilesInDir( selectImportDir );
    wxString      selectExportDir = m_frame->OpenDirDialog( "Select Export Direction" );
    ...
    for (int i=0;i<files.size();i++)
    {
        ...
        {
            FPT.importLCFP( files[i].ToStdString(), 1 );
        }
        else
        {
            FPT.importLCFP( files[i].ToStdString() );
        }

        FOOTPRINT* fp = FPT.m_KicadFPObj;
        if( fp )
        {
            m_frame->ExportFootprintNoPopDialog( fp, selectExportDir );

            //数据结构加载封装
            m_frame->AddFootprintToBoard( fp );
            m_frame->SetMsgPanel( fp );
            m_frame->PlaceFootprint( fp );
            fp->SetPosition( VECTOR2I( 0, 0 ) );
            m_frame->GetBoard()->BuildListOfNets();
            m_frame->UpdateView();
            m_frame->Zoom_Automatique( false );
            m_frame->RefreshCanvas();
            ...
            // //文件加载FP
            //wxString fileFullPath = selectExportDir + "\\" + fp->GetFPID().GetLibItemName();
            //fileFullPath += ".kicad_mod";
            //m_frame->ImportFootprint( fileFullPath );
            //m_frame->Zoom_Automatique( false );
            //m_frame->RefreshCanvas();

            std::thread saveImage( SaveCanvasImageToFile, m_frame,
                                   selectExportDir + "/" + fp->GetFPID().GetLibItemName() + ".jpg",
                                   BITMAP_TYPE::JPG );
            saveImage.detach();
            m_frame->GetCanvas()->ClearBackground();
            m_frame->GetCanvas()->ClearDebugOverlay();

            /*bool saveResult = SaveCanvasImageToFile(
                    m_frame, selectExportDir + "/" + fp->GetFPID().GetLibItemName() + ".jpg", BITMAP_TYPE::JPG );*/

        }
    }
    
    wxString msg = wxString::Format( _( "Footprint exported to file End" ) );
...
void FOOTPRINT_EDITOR_CONTROL::setTransitions()
{
    ...
    Go( &FOOTPRINT_EDITOR_CONTROL::AnalyseLCEDAFP, PCB_ACTIONS::analyseLCEDAFP.MakeEvent() );
    Go( &FOOTPRINT_EDITOR_CONTROL::MultiAnalyseLCEDAFP, PCB_ACTIONS::multiAnalyseLCEDAFP.MakeEvent() );
