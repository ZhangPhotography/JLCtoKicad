...
wxFileName FOOTPRINT_EDIT_FRAME::OpenFileDialog(const wxString& aLastPath )
{
    static int lastFilterIndex = 0; // To store the last choice during a session.
    wxString   wildCard;

    wildCard << "LCFP(*.efoo)|*.efoo" << wxChar( '|' ) << "LCFPJson(*.json)|*.json" << wxChar( '|' )
             << " AllFiles( *.*)|*.*";

    wxFileDialog dlg( this, _( "Import LCEDAFP" ), aLastPath, wxEmptyString, wildCard,
                      wxFD_OPEN | wxFD_FILE_MUST_EXIST );
...
wxString FOOTPRINT_EDIT_FRAME::OpenDirDialog(wxString strTip)
{
    wxDirDialog dlg( nullptr, strTip, "D:",
...
wxArrayString FOOTPRINT_EDIT_FRAME::GetAllFilesInDir(wxString strDir)
{
    wxDir dir;
...
//Export single FP( -sxl)
void FOOTPRINT_EDIT_FRAME::ExportFootprint( FOOTPRINT* aFootprint,wxString &fileSaveFullPath )
{
    wxFileName                 fn;
    FOOTPRINT_EDITOR_SETTINGS* cfg = GetSettings();
    ...
    wxFileDialog dlg( this, _( "Export Footprint" ), fn.GetPath(), fn.GetFullName(), wildcard,
    ...
    fn = EnsureFileExtension( dlg.GetPath(), KiCadFootprintFileExtension );
    cfg->m_LastImportExportPath = fn.GetPath();
    fileSaveFullPath = fn.GetFullPath();
    try
    {
        // Export as *.kicad_pcb format, using a strategy which is specifically chosen
        // as an example on how it could also be used to send it to the system clipboard.
        ...
        */
        ...
        {
            DisplayErrorMessage(
                    this, wxString::Format( _( "Insufficient permissions to write file '%s'." ),
                                            dlg.GetPath() ) );
            return;
        }
...
//Export FP£¨NO pop Dialog -sxl£©
void FOOTPRINT_EDIT_FRAME::ExportFootprintNoPopDialog( FOOTPRINT* aFootprint ,wxString outDir)
{
    wxFileName                 fn;
    FOOTPRINT_EDITOR_SETTINGS* cfg = GetSettings();
    ...
    fn = EnsureFileExtension( fn.GetPath(), KiCadFootprintFileExtension );
    cfg->m_LastImportExportPath = fn.GetPath();

    wxString fileFullPath = outDir + "\\" + aFootprint->GetFPID().GetLibItemName();
    fileFullPath = fileFullPath + "." + KiCadFootprintFileExtension;
    try
    {
        // Export as *.kicad_pcb format, using a strategy which is specifically chosen
        // as an example on how it could also be used to send it to the system clipboard.
        ...
        */
        ...
        FILE* fp = wxFopen( fileFullPath, wxT( "wt" ) );
    ...
    wxString msg = wxString::Format( _( "Footprint exported to file '%s'." ), fileFullPath );
    //DisplayInfoMessage( this, msg );
}
